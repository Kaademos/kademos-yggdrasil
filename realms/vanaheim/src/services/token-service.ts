 ...
 * VULNERABLE: Uses predictable PRNG based on timestamp
 * 
 * This service intentionally demonstrates weak randomness in token generation.
 * Attackers can analyze token patterns and predict future tokens.
 */
...
export class TokenService {
  ...
  /**
   * Generate token with WEAK PRNG
   * VULNERABLE: Uses timestamp-based seed which is predictable
   */
  generateToken(userId: string): Token {
    const timestamp = Date.now();
    const userIdNum = this.parseUserId(userId);
    
    // VULNERABLE: Predictable seed based on timestamp and userId
    const seed = timestamp + userIdNum * this.seedMultiplier;
    
    // Generate token using weak PRNG
    ...
    // Store in history (VULNERABLE: exposes pattern data)
   ...
   * VULNERABLE: Simple linear congruential generator with predictable output
   */
  ...
  /**
   * Get token history
   * VULNERABLE: Exposes token patterns including timestamps and seeds
   */
